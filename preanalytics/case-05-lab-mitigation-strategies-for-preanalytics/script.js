...
const flagButtons = Array.from(document.querySelectorAll(".flag-button"));
const flagPanels = Array.from(document.querySelectorAll("[data-flag-panel]"));

const storageKey = "case5LabMitigationProgress";
...
function switchPanel(buttons, panels, targetId, targetAttr) {
  buttons.forEach((button) => {
    const isActive = button.dataset[targetAttr] === targetId;
...
caseTabs.forEach((tab) => {
  tab.addEventListener("click", () => switchPanel(caseTabs, casePanels, tab.dataset.caseTarget, "caseTarget"));
});

flagButtons.forEach((button) => {
  button.addEventListener("click", () => switchPanel(flagButtons, flagPanels, button.dataset.flagTarget, "flagTarget"));
});
